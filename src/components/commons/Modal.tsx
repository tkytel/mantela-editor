...
export function Modal({ children, footer, id, isOpen, onClose, title }: ModalProps) {
	if (!isOpen) return null;

	return (
		<div
			className="fixed inset-0 z-50 flex h-full w-full items-center justify-center overflow-y-auto bg-gray-500/50 dark:bg-gray-900/75"
			id={id}
			role="presentation"
		>
			<div
				...
				aria-modal={isOpen}
				className="relative m-4 w-full max-w-md rounded-lg bg-white shadow-lg dark:bg-gray-800"
				role="dialog"
			>
				<div className="flex items-center justify-between border-b border-gray-200 p-4 dark:border-gray-600">
					<h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100" id={`${id}-modal-title`}>
						{title}
					</h3>
					<button
